    ...
    partial class Internal
    {
        ...
        internal static class GroupByHelper<TKey, TElement>
        {
                    ...
                    try
                    {
                        ...
                        using (var lookup = new LookupImpl<TKey, TElement, TEqualityComparer>(_comparer, true))
                        {
                            do
                            {
                                ...
                                var key = await _keySelector.Invoke(item, cancelationToken);
                                var group = lookup.GetOrCreateGrouping(key, true);

                                var element = await _elementSelector.Invoke(item, cancelationToken);
                                group.Add(element);
            ...
            internal static AsyncEnumerable<Linq.Grouping<TKey, TElement>> GroupBy<TSource, TKeySelector, TElementSelector, TEqualityComparer>(
                ...
                where TElementSelector : IFunc<TSource, CancelationToken, Promise<TElement>>
                where TEqualityComparer : IEqualityComparer<TKey>
                => AsyncEnumerable<Linq.Grouping<TKey, TElement>>.Create(
                    ...
                    try
                    {
                        ...
                        using (var lookup = new LookupImpl<TKey, TElement, TEqualityComparer>(_comparer, true))
                        {
                            do
                            {
                                ...
                                var key = await _keySelector.Invoke(item, cancelationToken).ConfigureAwait(_configuredAsyncEnumerator.ContinuationOptions);
                                var group = lookup.GetOrCreateGrouping(key, true);
