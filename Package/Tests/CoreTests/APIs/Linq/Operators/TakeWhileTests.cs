...
namespace ProtoPromise.Tests.APIs.Linq
{
    public static class TakeWhileHelper
    {
            ...
            CancelationToken configuredCancelationToken = default)
        {
            ...
            {
                return async
                    ? source.TakeWhile(async x => predicate(x))
                    : source.TakeWhile(predicate);
            }
            else
            {
                return async
                    ? source.TakeWhile(capturedValue, async (cv, x) =>
                    {
            ...
            CancelationToken configuredCancelationToken = default)
        {
            ...
            {
                return async
                    ? source.TakeWhile(async (x, i) => predicate(x, i))
                    : source.TakeWhile(predicate);
            }
            else
            {
                return async
                    ? source.TakeWhile(capturedValue, async (cv, x, i) =>
                    {
                        Assert.AreEqual(capturedValue, cv);
                        return predicate(x, i);
                    })
                    : source.TakeWhile(capturedValue, (cv, x, i) =>
                    {
                        Assert.AreEqual(capturedValue, cv);
                        return predicate(x, i);
                    });
    ...
    public class TakeWhileTests
    {
        ...
        public void TakeWhile_NullArgumentThrows()
        {
            ...
            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(default(Func<int, Promise<bool>>)));

            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(captureValue, default(Func<string, int, bool>)));
            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(captureValue, default(Func<string, int, Promise<bool>>)));
            ...
            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(default(Func<int, Promise<bool>>)));
            ...
            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(captureValue, default(Func<string, int, Promise<bool>>)));

            enumerable.GetAsyncEnumerator().DisposeAsync().Forget();
        }
        [Test]
        public void TakeWhile_WithIndex_NullArgumentThrows()
        {
            var enumerable = AsyncEnumerable.Return(42);
            var captureValue = "captureValue";

            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(default(Func<int, int, bool>)));
            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(default(Func<int, int, Promise<bool>>)));

            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(captureValue, default(Func<string, int, int, bool>)));
            Assert.Catch<System.ArgumentNullException>(() => enumerable.TakeWhile(captureValue, default(Func<string, int, int, Promise<bool>>)));

            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(default(Func<int, int, bool>)));
            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(default(Func<int, int, Promise<bool>>)));

            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(captureValue, default(Func<string, int, int, bool>)));
            Assert.Catch<System.ArgumentNullException>(() => enumerable.ConfigureAwait(SynchronizationOption.Synchronous).TakeWhile(captureValue, default(Func<string, int, int, Promise<bool>>)));
        ...
        [Test]
        public void TakeWhile_Indexed_Empty(
            [Values] bool configured,
            [Values] bool async,
            [Values] bool captureValue)
        {
            Promise.Run(async () =>
            {
                var asyncEnumerator = AsyncEnumerable<int>.Empty()
                    .TakeWhile(configured, async, captureValue, (x, i) => true)
                    .GetAsyncEnumerator();
                Assert.False(await asyncEnumerator.MoveNextAsync());
                await asyncEnumerator.DisposeAsync();
            }, SynchronizationOption.Synchronous)
            ...
            [Values] bool captureValue)
        {
            ...
            {
                var asyncEnumerator = new[] { 1, 2, 3, 4 }.ToAsyncEnumerable()
                    .TakeWhile(configured, async, captureValue, (x, i) => i < 2)
                    .GetAsyncEnumerator();
                Assert.True(await asyncEnumerator.MoveNextAsync());
                Assert.AreEqual(1, asyncEnumerator.Current);
                Assert.True(await asyncEnumerator.MoveNextAsync());
        ...
        [Test]
        public void TakeWhile_Indexed_False(
            [Values] bool configured,
            [Values] bool async,
            [Values] bool captureValue)
        {
            ...
            {
                var asyncEnumerator = new[] { 1, 2, 3, 4 }.ToAsyncEnumerable()
                    .TakeWhile(configured, async, captureValue, (x, i) => false)
                    .GetAsyncEnumerator();
                Assert.False(await asyncEnumerator.MoveNextAsync());
                await asyncEnumerator.DisposeAsync();
            }, SynchronizationOption.Synchronous)
        ...
        [Test]
        public void TakeWhile_Indexed_True(
            [Values] bool configured,
            [Values] bool async,
            [Values] bool captureValue)
        {
        ...
        [Test]
        public void TakeWhile_Indexed_Throws_Source(
            [Values] bool configured,
            [Values] bool async,
            [Values] bool captureValue)
        {
        ...
        [Test]
        public void TakeWhile_Indexed_Throws_Predicate(
            [Values] bool configured,
            [Values] bool async,
            [Values] bool captureValue)
        {
            Promise.Run(async () =>
            {
                var ex = new Exception("Bang!");
                var asyncEnumerator = new[] { 1, 2, 3, 4 }.ToAsyncEnumerable()
                    .TakeWhile(configured, async, captureValue, (x, i) => { throw ex; })
                    .GetAsyncEnumerator();
                await TestHelper.AssertThrowsAsync(() => asyncEnumerator.MoveNextAsync(), ex);
                Assert.False(await asyncEnumerator.MoveNextAsync());
                await asyncEnumerator.DisposeAsync();
        ...
        [Test]
        public void TakeWhile_Indexed_Cancel(
            [Values] ConfiguredType configuredType,
            [Values] bool async,
            [Values] bool captureValue,
            [Values] bool enumeratorToken)
        {
            Promise.Run(async () =>
            {
                ...
                using (var configuredCancelationSource = CancelationSource.New())
                {
                    ...
                    {
                        var asyncEnumerator = xs
                            .TakeWhile(configuredType != ConfiguredType.NotConfigured, async, captureValue, (x, i) => true,
                                configuredType == ConfiguredType.ConfiguredWithCancelation ? configuredCancelationSource.Token : CancelationToken.None)
                            .GetAsyncEnumerator(enumeratorCancelationSource.Token);
                        Assert.True(await asyncEnumerator.MoveNextAsync());
                        Assert.AreEqual(0, asyncEnumerator.Current);
